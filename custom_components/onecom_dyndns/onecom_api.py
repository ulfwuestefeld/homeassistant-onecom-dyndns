...
class OneComAPI:
    ...
    # Prevents the integration from hanging indefinitely if One.com becomes
    ...
    def login(self) -> bool:
        ...
        try:
            response = self.session.get(
                self.ADMIN_URL, allow_redirects=True, timeout=self.REQUEST_TIMEOUT,
            )
            response.raise_for_status()
        except requests.RequestException as e:
            raise OneComAPIError(f"Failed to connect to One.com: {e}") from e
        ...
        try:
            ...
            raise OneComAPIError(f"Login request failed: {e}") from e
        ...
        try:
            verify_response = self.session.get(
                f"{self.ADMIN_URL}/", allow_redirects=True, timeout=self.REQUEST_TIMEOUT,
            )
    ...
    def _get_dns_records(self) -> dict:
        ...
        try:
            ...
            raise OneComAPIError(f"Failed to fetch DNS records: {e}") from e
        except ValueError as e:
            raise OneComAPIError(f"Invalid JSON response: {e}") from e

    def _find_record_id(self, subdomain: str, records: dict) -> tuple[str, str] | None:
    ...
    def get_domains(self) -> list[str]:
        ...
        try:
            response = self.session.get(
                f"{self.ADMIN_URL}/api/domains", timeout=self.REQUEST_TIMEOUT,
            )
            response.raise_for_status()
            data = response.json()
    ...
    def update_dns_record(self, subdomain: str, ip_address: str) -> bool:
        ...
        try:
            ...
            raise OneComAPIError(f"Failed to update DNS record: {e}") from e

    def update_all_subdomains(self, subdomains: list[str], ip_address: str) -> dict:
    ...
    def create_txt_record(self, subdomain: str, content: str, ttl: int = 600) -> str:
        ...
        try:
            ...
            if response.status_code >= 400:
                ...
                _LOGGER.debug("Create TXT record error response: %s", response_text)
                ...
                if "DNS_RECORD_CONFLICTING" in response_text or "ConflictingDnsRecordException" in response_text:
                    ...
                    if match and "same content" in response_text:
                        ...
                        _LOGGER.info("TXT record already exists with same content (ID: %s)", existing_id)
                        return existing_id
            ...
            raise OneComAPIError(f"Failed to create TXT record: {e}") from e
    ...
    def delete_txt_record(self, record_id: str) -> bool:
        ...
        try:
            response = self.session.delete(
                delete_url, headers=headers, timeout=self.REQUEST_TIMEOUT,
            )
            response.raise_for_status()
            ...
            raise OneComAPIError(f"Failed to delete DNS record: {e}") from e

    def find_txt_records(self, subdomain: str, exact_match: bool = False) -> list[dict[str, object]]:
    ...
    def cleanup_acme_records(self, subdomain: str = "_acme-challenge") -> int:
        ...
        for record in records:
            try:
                self.delete_txt_record(str(record["id"]))
                deleted_count += 1
            except OneComAPIError as e:
    ...
    def wait_for_dns_propagation(
        ...
        Uses a public DNS resolver to verify the record is visible.
